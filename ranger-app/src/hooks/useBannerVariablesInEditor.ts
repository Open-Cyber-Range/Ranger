import {type editor} from 'monaco-editor';
import {useState, useEffect, useCallback} from 'react';
import {useTranslation} from 'react-i18next';
import {type BannerVariable} from 'src/models/exercise';

export const useBannerVariablesInEditor
	= (selectedDeployment: string | undefined,
		 editorInstance: editor.IStandaloneCodeEditor | undefined) => {
	const {t} = useTranslation();
	const [bannerVariables, setBannerVariables] = useState<BannerVariable[]>([]);

	const defaultBannerVariables = useCallback(() => [
		{name: '{{exerciseName}}', content: t('emails.variables.exerciseName')},
	], [t]);

	const specificDeploymentVariables = useCallback(() => [
		{name: '{{exerciseName}}', content: t('emails.variables.exerciseName')},
		{name: '{{deploymentName}}', content: t('emails.variables.deploymentName')},
		{name: '{{username}}', content: t('emails.variables.username')},
	], [t]);

	useEffect(() => {
		if (!selectedDeployment || selectedDeployment === '') {
			setBannerVariables(defaultBannerVariables());
		} else if (selectedDeployment) {
			setBannerVariables(specificDeploymentVariables());
		}
	}, [selectedDeployment, t, defaultBannerVariables, specificDeploymentVariables]);

	const insertVariable = (variable: string) => {
		if (editorInstance) {
			const position = editorInstance.getPosition();
			if (!position) {
				return;
			}

			editorInstance.executeEdits('', [{
				range: {
					startLineNumber: position.lineNumber,
					startColumn: position.column,
					endLineNumber: position.lineNumber,
					endColumn: position.column,
				},
				text: variable,
			}]);
		}
	};

	return {bannerVariables, insertVariable};
};
